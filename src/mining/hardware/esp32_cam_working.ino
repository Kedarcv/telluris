...
// AI WebSocket for JSON commands (port 8080)
...
void onAIWebSocketEvent(uint8_t num, WStype_t type, uint8_t * payload, size_t length) {
  switch(type) {
    ...
    case WStype_CONNECTED:
      ...
      wsAI.sendTXT(num, "{\"type\":\"connected\",\"message\":\"AI Control Ready\"}");
      break;
      
    case WStype_TEXT:
      Serial.printf("[AI] Received: %s\n", payload);
      processAICommand(String((char*)payload));
      break;
  }
}

// Process AI JSON commands
void processAICommand(String json) {
  ...
  if (type == "drive") {
    ...
    // Convert to motor commands (-1 to 1 range)
    ...
    int speed = (int)(abs(linear_mps) * 255);
    speed = constrain(speed, 0, 255);
    ledcWrite(PWMSpeedChannel, speed);
    
    Serial.printf("[AI] Drive command: linear=%.2f, angular=%.2f, speed=%d\n", linear_mps, angular_rps, speed);
    
  } else if (type == "stop") {
    moveCar(STOP);
    Serial.println("[AI] Stop command");
    
  } else if (type == "io") {
    // Digital I/O control (e.g., for lights)
    int pin = doc["pin"] | -1;
...
void loop() 
{
  ...
  // Camera refresh rate control - 1 second (1000ms) interval
  static unsigned long lastCameraUpdate = 0;
  unsigned long currentMillis = millis();
  
  if (currentMillis - lastCameraUpdate >= 1000) {
    sendCameraPicture();
